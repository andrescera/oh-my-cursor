...
| hephaestus | gpt-5.5-extra-high | Sustained deep work on one complex problem until end-to-end completion. |
...
| momus | gpt-5.5-extra-high | Reviews plans for executability and completeness; read-only. |
| multimodal-looker | gemini-3.1-pro | PDFs, images, diagrams when plain-text read is not enough. |
| oracle | gpt-5.5-extra-high | Architecture and debugging consultation; read-only. |
| prometheus | claude-opus-4-7-thinking-xhigh | Detailed planning only; never implements. |
| sisyphus-junior | composer-2-fast | Small bounded tasks; leaf worker; no sub-delegation. |
| sisyphus | claude-opus-4-7-thinking-xhigh | Main coordinator for complex multi-file work and execution. |
