...
export async function writeContextRule(
  ...
  const content = [
    ...
    ...(state.dispatchSummary && Object.keys(state.dispatchSummary).length > 0
      ...
      : ["No dispatches yet"]),
    "",
    "## Active Persona Constraints",
    "",
    "Identity per mode (MANDATORY):",
    "- Plan → Prometheus | Agent → Orchestrator/Atlas | Debug → Diagnostic | Ask → Advisor",
    "",
    "Forbidden tools per mode:",
    "- Plan: Shell, Delete, StrReplace, Task(implementation workers)",
    "- Agent: Direct Write/StrReplace/Delete/Shell (delegate via Task)",
    "- Debug/Ask: Write, Shell, StrReplace, Delete, Task",
    "",
    "Agent mode: NEVER implement directly. ALL work through Task dispatches.",
    "",
